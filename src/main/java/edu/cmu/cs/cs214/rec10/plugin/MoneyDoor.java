...
class MoneyDoorPlugin implements GamePlugin {
  ...
  public void onNewGame() {
    ...
    int i;
    // For every position i, chooses a random position >= i and swaps it with the value at position
    // i,
    ...
    for (i = 0; i < 10; i++) {
      ...
      while ((nextIndex = random.nextInt(10)) < i)
        ; // choosing a random number in the range [i, 10).
  ...
  public void onMovePlayed(int x, int y) {
    int money = arr[x];
    PlayerScores.put(
        framework.getCurrentPlayer().getName(),
